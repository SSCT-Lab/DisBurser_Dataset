    ...
    synchronized private SaslClient createSaslClient(final String servicePrincipal,
        ...
        try {
            ...
            if (subject.getPrincipals().isEmpty()) {
                ...
                Object[] publicCredentials = subject.getPublicCredentials().toArray();
                // username can not be empty
                if (0 == publicCredentials.length) {
                    throw new LoginException("username is empty.");
                }
                String username = (String) (publicCredentials[0]);

                Object[] privateCredentials = subject.getPrivateCredentials()
                        .toArray();
                // password can be empty
                String password = "";
                if (0 != privateCredentials.length) {
                    password = (String) (privateCredentials[0]);
                }
